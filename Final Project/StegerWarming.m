function [ FluxRight ] = StegerWarming( Wcurrent,Wnext,epsilon )
%Justin ChanWoo Yang
%260368098
%
%INUPUT
...
positiveDmatrix = [(uCurrent+sqrt(uCurrent^2+epsilon^2))/2 0 0; 0 ((uCurrent+cCurrent)+...
    sqrt((uCurrent+cCurrent)^2+epsilon^2))/2 0; 0 0 ((uCurrent-cCurrent)+sqrt((uCurrent-cCurrent)^2+epsilon^2))/2];
...
negativeDmatrix = [(uNext-sqrt(uNext^2+epsilon^2))/2 0 0; 0 ((uNext+cNext)-...
    sqrt((uNext+cNext)^2+epsilon^2))/2 0; 0 0 ((uNext-cNext)-sqrt((uNext-cNext)^2+epsilon^2))/2];
