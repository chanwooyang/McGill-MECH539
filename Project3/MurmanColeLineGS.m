function [ residual, time, numberOfIteration ] = MurmanColeLineGS( Mfree )
%Justin ChanWoo Yang
%260368098
...
Amatrix = zeros((GridXBeforeLE+GridXAlongAF+GridXAfterTE)*numberOfGridY,(GridXBeforeLE+GridXAlongAF+GridXAfterTE)*numberOfGridY);
bMatrix = zeros((GridXBeforeLE+GridXAlongAF+GridXAfterTE)*numberOfGridY);
...
while (numberOfIteration < maxIteration)
    ...
    for i=3:(GridXBeforeLE+GridXAlongAF+GridXAfterTE-1)
        for j=2:(numberOfGridY-1)
            ...
            %my colleague's method
%             rhs = -g*p(j,i-2)-d*p(j,i-1)-e*p(j,i+1);
%             p(j,i) = ThomasSolver(a,b,c,rhs);

            %Creating coefficient matrix A and matrix b
            for m = 1:((GridXBeforeLE+GridXAlongAF+GridXAfterTE)*numberOfGridY)
                if (m==1 || mod(m,numberOfGridY)==1)
                    Amatrix(m,m)=a;
                    Amatrix(m,m+1)=c;
                elseif (mod(m,numberOfGridY)==0)
                    Amatrix(m,m)=a;
                    Amatrix(m,m-1)=b;
                else
                    Amatrix(m,m)=a;
            ...
            for m = 1:((GridXBeforeLE+GridXAlongAF+GridXAfterTE)*numberOfGridY)
                if (m==1 || mod(m,numberOfGridY)==1)
                    bMatrix(m)=-g*p(j,i-2)-d*p(j,i-1)-e*p(j,i+1)-b*p(j+1,i);
                elseif (mod(m,numberOfGridY)==0)
                    bMatrix(m)=-g*p(j,i-2)-d*p(j,i-1)-e*p(j,i+1)-c*p(j-1,i);
                else
                    bMatrix(m)=-g*p(j,i-2)-d*p(j,i-1)-e*p(j,i+1);
                end
            end
            
            %Use Thomas algorithm to solve triangular matrix
            solution = Thomas(Amatrix,bMatrix);
            
            index = 0;
            for k = 1:numberOfGridY
                for l = 1:(GridXBeforeLE+GridXAlongAF+GridXAfterTE)
                    index = index+1;
                    p(j,i) = solution(index);
                end
            end
        end
    end
    ...
    fprintf('%i\n',numberOfIteration);      %To see how fast each iteration
